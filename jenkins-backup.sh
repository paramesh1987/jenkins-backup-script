...
function backup_jobs() {
  ...
  if [ -d "${run_in_path}" ]; then
    ...
    find . -maxdepth 1 -type d | while read job_name; do
      ...
      find "${JENKINS_HOME}/jobs/${rel_depth}/${job_name}/" -maxdepth 1  \( -name "*.xml" -o -name "nextBuildNumber" \) -print0 | xargs -0 -I {} cp -R {} "${ARC_DIR}/jobs/${rel_depth}/${job_name}/"
...
function main() {
  ...
  cp -R "${JENKINS_HOME}/"*.xml "${ARC_DIR}"

  cp -R "${JENKINS_HOME}/plugins/"*.[hj]pi "${ARC_DIR}/plugins"
  ...
  if [ ${hpi_pinned_count} -ne 0 -o ${jpi_pinned_count} -ne 0 ]; then
    cp -R "${JENKINS_HOME}/plugins/"*.[hj]pi.pinned "${ARC_DIR}/plugins"
  fi
